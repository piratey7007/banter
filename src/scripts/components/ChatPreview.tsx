...
export default function ChatPreview({ chat }: { chat: Chat }) {
  ...
  if (!lastMessage.senderId) setSender('New Chat')
    ...
    getDoc(doc(db, 'users', lastMessage.senderId)).then((doc) =>
      setSender(doc?.data()?.displayName),
    )
    ...
    if (!chat.name) {
      ;(async function getChatName() {
        ...
        async function getDisplayName(n: number) {
          return (await getDoc(doc(db, 'users', chatUsers[n]))).data()
            ?.displayName
        }
      })()
    } else {
