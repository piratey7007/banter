...
export default function ChatPreview({ chat }: { chat: Chat }) {
	...
	const [sender, setSender]: [string | null, Dispatch<SetStateAction<string | null>>] = useState<string | null>(null)
	const [chatName, setChatName]: [string | null, Dispatch<SetStateAction<string | null>>] = useState<string | null>(
		null,
	)
	...
	useEffect(() => {
		if (!chat.name) {
			;(async function getChatName() {
				...
				function getChatString(usernames: string[]) {
				...
				async function getDisplayName(n: number) {
