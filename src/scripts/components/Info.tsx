...
export default function Info() {
	...
	async function handleSubmit(e: MouseEvent) {
		...
		const age = typeof ageRef.current?.value == 'string' ? parseInt(ageRef.current?.value) : ageRef.current?.value,
		...
		<Card>
			...
			<Form onSubmit={handleSubmit}>
				<div className='container flex flex-col justify-center items-center gap-2'>
					<UserImage colors={user?.defaultColors} url={user?.photoURL || undefined} />
