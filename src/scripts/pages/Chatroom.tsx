...
export default function Chatroom({ className, ...props }: { className?: string }) {
	const { id } = useParams()
	if (!id) return <div>Chatroom not found</div>
	const chatroomRef = useRef<HTMLDivElement>(null)
	const { user } = useContext(AuthContext)
	const [messages, getMessages, addMessage] = useMessages(id) as [MessageI[], Function, Function]
	useEffect(() => {
		;(async () => {
			await getMessages()
	...
	const images: Map<string | undefined, { photoURL?: string; defaultColors?: string[] | string }> = new Map([
		[user?.uid, { photoURL: user?.photoURL || undefined, defaultColors: user?.defaultColors }],
	])

	const getSenderPhoto = async (sender: string) => {
		if (!sender) return {}
		if (images.has(sender)) return images.get(sender)
		let photo: { photoURL?: string; defaultColors?: string[] | string } | undefined
		if (sender == user?.uid && (user.photoURL || user?.defaultColors))
			photo = { photoURL: user?.photoURL || undefined, defaultColors: user?.defaultColors || undefined }
		else {
