...
export default function useTimestamp(input: { seconds: number; nanoseconds: number }, type = 'relative') {
	const [timestamp, setTimestamp]: [string, (newTimestamp: string) => void] = useState('')
	let getTimestamp
	if (type === 'relative') {
		getTimestamp = () => {
			// Gets the time since the message has been sent
			let t = Math.floor(Date.now() / 1000 - input.seconds - input.nanoseconds / 1000000000)
			let stamp
			...
			if (t < 2629746) return setTimestamp(`${Math.floor(t / 604800)} weeks ago`)
			if (t < 5259492) return setTimestamp(`1 month ago`)
			if (t < 31556952) return setTimestamp(`${Math.floor(t / 2629746)} months ago`)
			if (t < 63113904) return setTimestamp(`1 year ago`)
			if (t < 10 * 631139040) return setTimestamp(`${Math.floor(t / 31556952)} years ago`)
		...
		getTimestamp = () => {
			...
			if (date.toDateString() === new Date().toDateString())
				stamp = date.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })
			// if date is yesterday, show "Yesterday"
			else if (date.toDateString() === new Date(Date.now() - 86400000).toDateString()) stamp = 'Yesterday'
			// if date is within the last 7 days, show day of week
			else if (date.getTime() > Date.now() - 604800000) stamp = date.toLocaleString('en-US', { weekday: 'short' })
